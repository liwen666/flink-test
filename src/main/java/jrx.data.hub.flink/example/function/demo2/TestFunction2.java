...
public class TestFunction2 {
    public static void main(String[] args) throws Exception {
        ...
        String ddlSource = "CREATE TABLE `file_incr_txn_bm_acct_loan` (\n" +
                ...
                "        'path'='D:\\flink12\\flink-1.12.1-src\\flink-engine\\src\\main\\resources\\increment.txt',\n" +
                "        'format'='maxwell-json'\n" +
                "        )";
        blinkStreamTableEnv.registerFunction("split", new Split(" "));
        blinkStreamTableEnv.registerFunction("duplicator", new DuplicatorFunction());
        blinkStreamTableEnv.registerFunction("flatten", new FlattenFunction());
        String job1 = "select * from file_incr_txn_bm_acct_loan ";
        String job2 = " select * from file_incr_txn_bm_acct_loan where txa_number='14047254894588067025'";

        blinkStreamTableEnv.executeSql(ddlSource);
        TableResult tableResult = blinkStreamTableEnv.executeSql(job1);
        tableResult.print();
        TableResult table2 = blinkStreamTableEnv.executeSql(job2);
        table2.print();
