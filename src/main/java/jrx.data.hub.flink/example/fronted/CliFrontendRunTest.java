...
package jrx.data.hub.flink.example.fronted;
...
public class CliFrontendRunTest extends CliFrontendTestBase {
    ...
    public void testRun() throws Exception {
        ...
        {
            String[] parameters = {"-v", getTestJarPath(null)};
        ...
        {
            String[] parameters = {"-v", "-d", getTestJarPath(null)};
            verifyCliFrontend(configuration, getCli(), parameters, 4, true);
        }

        // test configure parallelism
        {
            String[] parameters = {"-v", "-p", "42", getTestJarPath(null)};
            verifyCliFrontend(configuration, getCli(), parameters, 42, false);
        }

        // test detached mode
        {
            String[] parameters = {"-p", "2", "-d", getTestJarPath(null)};
            verifyCliFrontend(configuration, getCli(), parameters, 2, true);
        }

        // test configure savepoint path (no ignore flag)
        {
            String[] parameters = {"-s", "expectedSavepointPath", getTestJarPath(null)};

            CommandLine commandLine =
                    CliFrontendParser.parse(CliFrontendParser.getRunCommandOptions(), parameters, true);
        ...
        {
            String[] parameters = {"-s", "expectedSavepointPath", "-n", getTestJarPath(null)};
        ...
        {
            String[] parameters = {
                getTestJarPath(null), "-arg1", "value1", "justavalue", "--arg2", "value2"
            };
    ...
    public void testParallelismWithOverflow() throws Exception {
        ...
        String[] parameters = {"-v", "-p", "475871387138", getTestJarPath(null)};
    ...
    public static void verifyCliFrontend(
        ...
        RunTestingCliFrontend testFrontend =
                new RunTestingCliFrontend(
                        ...
                        expectedParallelism,
                        isDetached);
        testFrontend.parseAndRun(parameters); // verifies the expected values (see below)
    }
