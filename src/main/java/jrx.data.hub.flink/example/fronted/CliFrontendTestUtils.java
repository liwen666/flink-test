...
public class CliFrontendTestUtils {
    ...
    public static String getTestJarPath(String fileName) throws FileNotFoundException, MalformedURLException {
        String projectBaseDir = System.getProperty("project.basedir");
        Path testJarPath = Paths.get(projectBaseDir, "target",fileName==null?"SocketWindowWordCount.jar":fileName);
    ...
    public static String getSampleCfg() {
        String confFile =
                CliFrontendRunTest.class.getResource("/sample/flink-conf.yaml").getFile();
        return new File(confFile).getAbsoluteFile().getParent();
    }
